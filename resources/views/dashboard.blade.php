...
@section('title', __('Dashboard Payroll & HR Analytics - Kimia Farma'))
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-xl font-extrabold text-slate-900 tracking-tight">{{ __('Executive Payroll Dashboard') }}</h2>
            <p class="text-xs text-slate-500">{{ __('Ringkasan finansial dan status penggajian karyawan Kimia Farma Group.') }}</p>
        </div>
        ...
        <div class="flex items-center gap-2">
            <button class="flex-1 sm:flex-none px-4 py-2 bg-white border border-slate-200 text-slate-700 text-xs font-semibold rounded-xl hover:bg-slate-50 transition shadow-sm flex items-center justify-center gap-2">
                <i class="fa-solid fa-file-export text-slate-500"></i> {{ __('Ekspor Laporan') }}
            </button>
            <a href="#" class="flex-1 sm:flex-none px-4 py-2 bg-kf-navy text-white text-xs font-semibold rounded-xl hover:bg-kf-navyDark transition shadow-md shadow-blue-900/10 flex items-center justify-center gap-2">
                <i class="fa-solid fa-bolt text-kf-cyan"></i> {{ __('Process Payroll Run') }}
            </a>
        </div>
    </div>

    <!-- ================= KPI CARDS (KOLOM BIRU / STATISTIK) ================= -->
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm relative overflow-hidden">
            <div class="flex items-center justify-between mb-3">
                <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">{{ __('Beban Payroll Bulan Ini') }}</span>
            ...
            <div class="mt-2 flex items-center gap-2 text-[11px]">
                ...
                <span class="text-slate-400">{{ __('vs bulan lalu') }}</span>
            </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm relative overflow-hidden">
            <div class="flex items-center justify-between mb-3">
                <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">{{ __('Status Pembayaran Gaji') }}</span>
            ...
            <h3 class="text-xl font-extrabold text-slate-900">3.248 <span class="text-xs font-normal text-slate-400">/ 3.421 {{ __('Karyawan') }}</span></h3>
            <div class="mt-2 flex items-center gap-2 text-[11px]">
                <span class="text-amber-500 font-bold">173 {{ __('Pending') }}</span>
                <span class="text-slate-400">| 95% {{ __('Selesai') }}</span>
            </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm relative overflow-hidden">
            <div class="flex items-center justify-between mb-3">
                <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">{{ __('Estimasi PPh 21 (TER)') }}</span>
            ...
            <div class="mt-2 flex items-center gap-2 text-[11px]">
                <span class="text-slate-500 font-medium">{{ __('Potongan Pajak Resmi') }}</span>
            </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm relative overflow-hidden">
            <div class="flex items-center justify-between mb-3">
                <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">{{ __('Iuran BPJS (Kes & TK)') }}</span>
            ...
            <div class="mt-2 flex items-center gap-2 text-[11px]">
                <span class="text-slate-500 font-medium">{{ __('Beban Perusahaan + Karyawan') }}</span>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-8 bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm space-y-4">
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="font-bold text-slate-800 text-sm">{{ __('Tren Pengeluaran Payroll (12 Bulan)') }}</h3>
                    <p class="text-[11px] text-slate-400">{{ __('Evolusi total beban gaji bersih Kimia Farma Group.') }}</p>
                </div>
                <span class="text-xs font-semibold text-kf-navy bg-blue-50 px-2.5 py-1 rounded-lg border border-blue-100">{{ __('Tahun 2026') }}</span>
            </div>
            <div class="h-64">
        ...
        <div class="lg:col-span-4 bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm space-y-4 flex flex-col justify-between">
            <div>
                <h3 class="font-bold text-slate-800 text-sm">{{ __('Breakdown Komponen') }}</h3>
                <p class="text-[11px] text-slate-400">{{ __('Proporsi alokasi komponen gaji bulan ini.') }}</p>
            </div>
            ...
            <div class="grid grid-cols-2 gap-2 text-[11px] pt-2 border-t border-slate-100">
                <div class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-full bg-kf-navy"></span> {{ __('Gaji Pokok') }} (65%)</div>
                <div class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-full bg-kf-cyan"></span> {{ __('Tunjangan') }} (20%)</div>
                <div class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-full bg-amber-500"></span> {{ __('Lembur / Overtime') }} (10%)</div>
                <div class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-full bg-emerald-500"></span> {{ __('Insentif') }} (5%)</div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
        <div class="p-5 border-b border-slate-100 flex items-center justify-between">
            <div>
                <h3 class="font-bold text-slate-800 text-sm">{{ __('Aktivitas Payroll & Approval Terakhir') }}</h3>
                <p class="text-[11px] text-slate-400">{{ __('Pengajuan lembur, penyesuaian insentif apoteker, dan status persetujuan.') }}</p>
            </div>
            <a href="#" class="text-xs font-bold text-kf-navy hover:text-kf-cyan transition">{{ __('Lihat Semua Logs') }} →</a>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-xs text-slate-600">
                <thead class="bg-slate-50 text-slate-500 font-bold uppercase tracking-wider text-[10px] border-b border-slate-100">
                    <tr>
                        <th class="py-3.5 px-6">{{ __('Karyawan / NIP') }}</th>
                        <th class="py-3.5 px-4">{{ __('Unit Kerja') }}</th>
                        <th class="py-3.5 px-4">{{ __('Jenis Transaksi') }}</th>
                        <th class="py-3.5 px-4">{{ __('Nominal / Nilai') }}</th>
                        <th class="py-3.5 px-4">{{ __('Tanggal Pengajuan') }}</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    <tr class="hover:bg-slate-50/80 transition">
                        ...
                        <td class="py-3.5 px-4"><span class="bg-blue-50 text-kf-navy px-2 py-0.5 rounded font-semibold text-[10px]">{{ __('Insentif Shift Malam') }}</span></td>
                        <td class="py-3.5 px-4 font-bold text-slate-800">Rp 1.250.000</td>
                        <td class="py-3.5 px-4">18 Agu 2026</td>
                        <td class="py-3.5 px-4"><span class="bg-amber-100 text-amber-700 font-bold px-2.5 py-1 rounded-full text-[10px]">{{ __('Pending Approval') }}</span></td>
                        <td class="py-3.5 px-6 text-right">
                            <button class="px-2.5 py-1 bg-kf-navy text-white rounded-lg text-[10px] hover:bg-kf-navyDark transition">{{ __('Review') }}</button>
                        </td>
                    </tr>
                    ...
                    <tr class="hover:bg-slate-50/80 transition">
                        ...
                        <td class="py-3.5 px-4"><span class="bg-purple-50 text-purple-700 px-2 py-0.5 rounded font-semibold text-[10px]">{{ __('Tunjangan Bahaya Kimia') }}</span></td>
                        <td class="py-3.5 px-4 font-bold text-slate-800">Rp 3.000.000</td>
                        <td class="py-3.5 px-4">17 Agu 2026</td>
                        <td class="py-3.5 px-4"><span class="bg-emerald-100 text-emerald-700 font-bold px-2.5 py-1 rounded-full text-[10px]">{{ __('Approved') }}</span></td>
                        <td class="py-3.5 px-6 text-right">
                            <button class="px-2.5 py-1 bg-slate-100 text-slate-600 rounded-lg text-[10px] hover:bg-slate-200 transition">{{ __('Detail') }}</button>
                        </td>
                    </tr>
                    ...
                    <tr class="hover:bg-slate-50/80 transition">
                        ...
                        <td class="py-3.5 px-4"><span class="bg-amber-50 text-amber-700 px-2 py-0.5 rounded font-semibold text-[10px]">{{ __('Klaim Overtime (Lembur)') }}</span></td>
